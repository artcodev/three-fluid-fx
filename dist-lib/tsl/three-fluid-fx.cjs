...
function makeStorageTarget(width, height, linear, name) {
  const texture = new webgpu.StorageTexture(width, height);
  texture.name = name;
  texture.format = webgpu.RGBAFormat;
  texture.type = webgpu.HalfFloatType;
  texture.generateMipmaps = false;
  texture.mipmapsAutoUpdate = false;
  if (!linear) {
    texture.minFilter = webgpu.NearestFilter;
    texture.magFilter = webgpu.NearestFilter;
  }
  return texture;
}
...
function resizeStorage(texture, width, height) {
...
class WGSLFluidSimulation {
  ...
  constructor(renderer, options = {}) {
    ...
    this.densityNode = tsl.texture(this.density.read);
    this.dyeNode = tsl.texture(this.dye.read);
    this.velocityNode = tsl.texture(this.velocity.read);
    this.pressureNode = tsl.texture(this.pressure.read);
    this.divergenceNode = tsl.texture(this.divergence);
    this.curlNode = tsl.texture(this.curl);
  ...
  clearTexture(encoder, texture, width, height) {
...
class ChromaticDistortionNode extends webgpu.TempNode {
  static get type() {
  ...
  setup(_builder) {
    ...
    const sceneUv = sceneTex.uvNode || tsl.uv();
    const fluidUv = fluidTex.uvNode || tsl.uv();
    const invSize = this._invSize;
    const intensity = this.intensityNode;
    const apply = tsl.Fn(() => {
      const offX = tsl.vec2(invSize.x.mul(2), 0);
      const offY = tsl.vec2(0, invSize.y.mul(2));
      ...
      const r = sceneTex.sample(distUv.add(tsl.vec2(chroma.x, chroma.y))).r;
      const g = sceneTex.sample(distUv.add(tsl.vec2(chroma.x.negate(), chroma.y))).g;
      const b = sceneTex.sample(distUv.add(tsl.vec2(chroma.x.negate(), chroma.y.negate()))).b;
      return tsl.vec4(r, g, b, 1);
    });
    return apply();
  }
}
const chromaticDistortion = (sceneNode, fluidNode, intensity = 1) => tsl.nodeObject(
  new ChromaticDistortionNode(
    tsl.convertToTexture(sceneNode),
    tsl.convertToTexture(fluidNode),
    tsl.nodeObject(intensity)
  )
);
class SimpleDistortionNode extends webgpu.TempNode {
  static get type() {
...
const simpleDistortion = (sceneNode, fluidNode, intensity = 1) => tsl.nodeObject(
...
class RGBShiftDistortionNode extends webgpu.TempNode {
  static get type() {
  ...
  setup(_builder) {
    ...
    const apply = tsl.Fn(() => {
      ...
      const speed = tsl.max(vel.length(), 1e-4);
...
const rgbShiftDistortion = (sceneNode, fluidNode, intensity = 1) => tsl.nodeObject(
...
class WaterDistortionNode extends webgpu.TempNode {
  static get type() {
  ...
  setup(_builder) {
    ...
    const apply = tsl.Fn(() => {
      ...
      const normal = tsl.vec2(hR.sub(hL), hU.sub(hD));
...
const waterDistortion = (sceneNode, fluidNode, intensity = 1) => tsl.nodeObject(
...
class WaterCausticsDistortionNode extends webgpu.TempNode {
  static get type() {
  ...
  setup(_builder) {
    ...
    const causticWeb = (rawUv, t) => {
      ...
      for (let n = 0; n < 5; n++) {
        ...
        const lenArg = tsl.vec2(p.x.div(denomX), p.y.div(denomY));
        c = c.add(tsl.float(1).div(lenArg.length()));
      }
      c = c.div(5);
      c = tsl.float(1.17).sub(c.pow(1.4));
    ...
    const apply = tsl.Fn(() => {
      ...
      const g = sceneTex.sample(sceneUv.add(offset)).g;
      const b = sceneTex.sample(sceneUv.add(offset.mul(1.05))).b;
      const surface = tsl.smoothstep(0.015, 0.16, hC);
      const slope = tsl.smoothstep(15e-4, 0.04, normal.length());
      const cuv = sceneUv.mul(4).add(vel.mul(12e-4));
      const web = causticWeb(cuv, timeNode.mul(0.5).add(23));
      const caustic = tsl.vec3(web, web, web).add(tsl.vec3(0, 0.35, 0.5)).clamp(0, 1);
      const energy = web.pow(1.25).mul(surface).mul(slope.mul(0.6).add(0.4));
      const color = tsl.vec3(r, g, b).add(caustic.mul(energy).mul(intensity).mul(0.38));
      return tsl.vec4(color, 1);
    });
    return apply();
  }
}
const waterCausticsDistortion = (sceneNode, fluidNode, intensity = 1, time = 0) => tsl.nodeObject(
...
const toNode$1 = (value, fallback) => tsl.nodeObject(value ?? fallback);
class DensityTintOverlayNode extends webgpu.TempNode {
  static get type() {
  ...
  setup(_builder) {
    ...
    const apply = tsl.Fn(() => {
      ...
      return tsl.vec4(scene.add(tint.mul(density).mul(intensity)), 1);
    });
    return apply();
  }
}
const densityTintOverlay = (sceneNode, fluidNode, options = {}) => tsl.nodeObject(
  new DensityTintOverlayNode(
    tsl.convertToTexture(sceneNode),
    tsl.convertToTexture(fluidNode),
    toNode$1(options.intensity, tsl.float(0.14)),
    toNode$1(options.tint, tsl.vec3(0.1, 0.42, 0.36))
  )
);
const TAU = 6.28318530718;
const toNode = (value, fallback) => tsl.nodeObject(value ?? fallback);
class FluidOverlayNode extends webgpu.TempNode {
  static get type() {
  ...
  setup(_builder) {
    ...
    const sceneUv = sceneTex.uvNode || tsl.uv();
    const fluidUv = densityTex.uvNode || tsl.uv();
    const intensity = this.intensityNode;
    const timeNode = this.timeNode;
    ...
    const sampleDye5 = (uv) => {
      ...
      d = d.add(dyeTex.sample(uv.add(texel.mul(tsl.vec2(1, 1)))).rgb.mul(0.125));
      d = d.add(dyeTex.sample(uv.add(texel.mul(tsl.vec2(-1, 1)))).rgb.mul(0.125));
      d = d.add(dyeTex.sample(uv.add(texel.mul(tsl.vec2(1, -1)))).rgb.mul(0.125));
      d = d.add(dyeTex.sample(uv.add(texel.mul(tsl.vec2(-1, -1)))).rgb.mul(0.125));
      return d;
    ...
    const oilPalette = (phase) => {
      const ember = tsl.vec3(1, 0.33, 0.2);
      const mint = tsl.vec3(0.08, 0.78, 0.68);
      const cream = tsl.vec3(1, 0.84, 0.55);
      return tsl.mix(
        tsl.mix(ember, cream, tsl.smoothstep(0.15, 0.85, phase)),
        mint,
        tsl.smoothstep(0.55, 1, phase).mul(0.42)
      );
    };
    const hsv2rgb2 = (c) => {
      const K = tsl.vec4(1, 2 / 3, 1 / 3, 3);
      const p = c.xxx.add(K.xyz).fract().mul(6).sub(K.www).abs();
      return c.z.mul(tsl.mix(K.xxx, p.sub(K.xxx).clamp(0, 1), c.y));
    };
    const vibrant = (color, amount) => {
      const lum = tsl.dot(color, tsl.vec3(0.299, 0.587, 0.114));
      return tsl.mix(tsl.vec3(lum, lum, lum), color, tsl.float(1).add(amount)).clamp(0, 1);
    };
    const farDye = () => {
      const offX = tsl.vec2(texel.x.mul(8), 0);
      const offY = tsl.vec2(0, texel.y.mul(8));
    ...
    const trailTail = (vel) => {
      let tail = tsl.float(0);
      let wsum = tsl.float(0);
      for (let i = 1; i < 8; i += 1) {
        const weight = 1 - i / 8;
    ...
    const apply = tsl.Fn(() => {
      ...
      const composite = (overlay) => tsl.vec4(tsl.mix(scene, overlay, opacity.clamp(0, 1)), 1);
      if (this.style === "default") {
        const core = tsl.smoothstep(0.02, 0.55, farDye().mul(intensity).mul(4));
        const kinetic = speed.mul(0.02).clamp(0, 1);
        const tint2 = vibrant(
          cursorColor.mul(tsl.float(0.48).add(core.mul(0.52))).add(tsl.vec3(kinetic.mul(0.12))),
          vibrance
        );
        const alpha = dyeAmp.mul(intensity).mul(3).clamp(0, 0.95);
        return composite(tsl.mix(scene, tint2, alpha));
      }
      if (this.style === "volumeCursor") {
        const offX = tsl.vec2(texel.x.mul(1.5), 0);
        const offY = tsl.vec2(0, texel.y.mul(1.5));
        const grad = tsl.vec2(
        ...
        const lit = tsl.dot(ndir, tsl.vec2(-0.6, 0.8)).mul(0.5).add(0.5).clamp(0.2, 1);
        const shade = dyeAmp.pow(0.42).mul(lit);
        const tint2 = vibrant(cursorColor.mul(tsl.float(0.3).add(shade.mul(1.1))).clamp(0, 1), vibrance);
      ...
      if (this.style === "colorful") {
        ...
        for (let i = 0; i < 6; i += 1) {
          ...
          const a = hsv2rgb2(tsl.vec3(hueA.fract(), 0.9, 1));
          const b = hsv2rgb2(tsl.vec3(hueB.fract(), 0.85, 0.95));
          color = color.add(tsl.mix(a, b, 0.5).mul(w));
        }
      ...
      if (this.style === "rainbowFish") {
        ...
        const angle = tsl.atan(fishVel.y, fishVel.x);
        ...
        const a = hsv2rgb2(tsl.vec3(hueA.fract(), 0.92, 1));
        const b = hsv2rgb2(tsl.vec3(hueB.fract(), 0.7, 0.95));
        const color = vibrant(tsl.mix(a, b, 0.35), vibrance);
      ...
      if (this.style === "glaze") {
        return composite(
          scene.add(
            vibrant(tsl.vec3(1, 0.45, 0.22), vibrance).mul(density).mul(intensity)
          )
        );
      }
      if (this.style === "burn") {
        let fingers = tsl.float(0);
        for (let i = 0; i < 5; i += 1) {
        ...
        const burnColor = tsl.vec3(1, 0.3, 0);
        const emberColor = tsl.vec3(0.8, 0.15, 0);
        let fireColor = tsl.mix(emberColor, burnColor, fingers.clamp(0, 1));
        fireColor = fireColor.add(burnColor.mul(fingers.clamp(0, 1).pow(2)).mul(2));
        const smoke = fingers.mul(0.3);
        fireColor = fireColor.add(tsl.vec3(0.1, 0.1, 0.15).mul(smoke));
        ...
        return composite(tsl.mix(scene, vibrant(fireColor, vibrance), alpha));
      }
      if (this.style === "smoke") {
        const alpha = dyeAmp.mul(intensity).mul(2.6).clamp(0, 0.78);
        const smoke = tsl.vec3(0.9, 0.92, 0.94).mul(tsl.float(0.22).add(alpha.mul(0.9)));
        return composite(tsl.mix(scene, smoke, alpha));
      }
      ...
      if (this.style === "rainbowInk") {
        ...
        const tint2 = vibrant(tsl.mix(dyeHue, palette(phase), 0.45), vibrance);
      ...
      if (this.style === "colorWater") {
        ...
        const wash = tsl.mix(scene, vibrant(dyeHue, vibrance), alpha);
...
const fluidOverlay = (style, sceneNode, densityNode, dyeNode, velocityNode, options = {}) => tsl.nodeObject(
  new FluidOverlayNode(
    style,
    tsl.convertToTexture(sceneNode),
    tsl.convertToTexture(densityNode),
    tsl.convertToTexture(dyeNode),
    tsl.convertToTexture(velocityNode),
    {
      intensity: toNode(options.intensity, tsl.float(1)),
      time: toNode(options.time, tsl.float(0)),
      texel: toNode(options.texel, tsl.vec2(1 / 512, 1 / 512)),
      cursorColor: toNode(options.cursorColor, tsl.vec3(0.85, 0.95, 1)),
      vibrance: toNode(options.vibrance, tsl.float(0)),
      velocityScale: toNode(options.velocityScale, tsl.float(1)),
      opacity: toNode(options.opacity, tsl.float(1))
    }
  )
);
...
exports.ChromaticDistortionNode = ChromaticDistortionNode;
exports.DensityTintOverlayNode = DensityTintOverlayNode;
exports.FLUID_PROFILES = FLUID_PROFILES;
exports.FluidOverlayNode = FluidOverlayNode;
exports.FluidSimulation = WGSLFluidSimulation;
exports.RGBShiftDistortionNode = RGBShiftDistortionNode;
exports.SimpleDistortionNode = SimpleDistortionNode;
exports.WGSLFluidSimulation = WGSLFluidSimulation;
exports.WaterCausticsDistortionNode = WaterCausticsDistortionNode;
exports.WaterDistortionNode = WaterDistortionNode;
exports.artInkOverlay = artInkOverlay;
exports.attachPointerSplats = attachPointerSplats;
exports.burnOverlay = burnOverlay;
exports.chromaticDistortion = chromaticDistortion;
exports.colorWaterOverlay = colorWaterOverlay;
exports.colorfulOverlay = colorfulOverlay;
exports.defaultOverlay = defaultOverlay;
exports.densityTintOverlay = densityTintOverlay;
exports.fluidOverlay = fluidOverlay;
exports.glazeOverlay = glazeOverlay;
exports.liquidLensOverlay = liquidLensOverlay;
exports.oilOverlay = oilOverlay;
exports.rainbowFishOverlay = rainbowFishOverlay;
exports.rainbowInkOverlay = rainbowInkOverlay;
exports.rgbShiftDistortion = rgbShiftDistortion;
exports.simpleDistortion = simpleDistortion;
exports.smokeOverlay = smokeOverlay;
exports.trailOverlay = trailOverlay;
exports.velocityOverlay = velocityOverlay;
exports.volumeCursorOverlay = volumeCursorOverlay;
exports.waterCausticsDistortion = waterCausticsDistortion;
exports.waterDistortion = waterDistortion;
//# sourceMappingURL=three-fluid-fx.cjs.map
