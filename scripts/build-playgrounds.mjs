...
function codeSandboxTasksForPlayground() {
  return {
    setupTasks: [
      {
...
async function writePlayground({ slug, entryFile }, spec) {
  ...
  await mkdir(path.join(outputDir, '.codesandbox'), { recursive: true })
  await writeFile(
    path.join(outputDir, '.codesandbox/tasks.json'),
    `${JSON.stringify(codeSandboxTasksForPlayground(), null, 2)}\n`,
  )
...
function rootReadmeForPlaygrounds(items, spec) {
  const rows = items
    .map(
      (item) =>
        `| ${item.title} | \`${item.id}\` | [CodeSandbox](${codeSandboxHref(item, spec)}) / [StackBlitz](${stackBlitzHref(item, spec)}) |`,
    )
    .join('\n')
