...
const blob = new Mesh(new IcosahedronGeometry(1.3, 2), new MeshNormalNodeMaterial())
scene.add(blob)
const fluid = new FluidSimulation(renderer, DEFAULTS)
attachPointerSplats(renderer.domElement, fluid)
const scenePass = pass(scene, camera)
const pipeline = new RenderPipeline(renderer)
setPipelineOutput(pipeline, simpleDistortion(asNode(scenePass), asNode(fluid.densityNode), 1))
const resize = () => {
