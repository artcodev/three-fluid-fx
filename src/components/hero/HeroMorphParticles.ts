...
} from '../../../examples/extras/particles/tsl/WGSLFlowParticles'

const HOLD_SECONDS = 6.5
const MORPH_SECONDS = 4.8
const SEGMENT_SECONDS = HOLD_SECONDS + MORPH_SECONDS
...
export class HeroMorphParticles {
  ...
  constructor(
    ...
    this.particles = createWGSLFlowParticles(this.renderer, {
      mode: 'cloud3d',
      size: this.size,
    })
  ...
  private updateDestination(timeSeconds: number): void {
    ...
    const cycleSeconds = SEGMENT_SECONDS * this.targets.length
    const phaseSeconds = timeSeconds % cycleSeconds
    const fromIndex = Math.floor(phaseSeconds / SEGMENT_SECONDS)
    const toIndex = (fromIndex + 1) % cycle
    const localSeconds = phaseSeconds - fromIndex * SEGMENT_SECONDS
    const progress =
      localSeconds <= HOLD_SECONDS
        ? 0
        : Math.min(1, Math.max(0, (localSeconds - HOLD_SECONDS) / MORPH_SECONDS))
