...
import { MorphFlowParticles } from '../../../extras/particles/tsl/MorphFlowParticles'
...
type OverlayStyle = FluidOverlayStyle
...
const OVERLAY_OPTIONS: Record<string, OverlayStyle> = {
  Default: 'default',
...
const DISTORTION_OPTIONS: Record<string, DistortionStyle> = {
  Simple: 'simple',
  'RGB Shift': 'rgbShift',
  Chromatic: 'chromatic',
  Water: 'water',
...
const OVERLAY_STYLE_DEFAULTS: Record<
  OverlayStyle,
  {
    intensity: number
...
const particles = new MorphFlowParticles(renderer, {
  ...
  morphSeconds: params.morphSeconds,
})
...
function buildDistortion(style: DistortionStyle, sceneNode: Node): Node {
...
function buildOverlay(style: OverlayStyle, sceneNode: Node): Node {
  return fluidOverlay(
    style,
    sceneNode,
...
const usesCursorColor = (style: OverlayStyle): boolean =>
  style === 'trail' || style === 'default' || style === 'volumeCursor' || style === 'artInk'

const usesVibrance = (style: OverlayStyle): boolean => style !== 'smoke' && style !== 'velocity'
const usesVelocityScale = (style: OverlayStyle): boolean =>
  style === 'velocity' || style === 'rainbowFish'

type VisibilityBinding = { hidden: boolean }

let overlayVelocityScaleBinding: VisibilityBinding | undefined
let cursorColorBinding: VisibilityBinding | undefined
let vibranceBinding: VisibilityBinding | undefined
let liquidColorBinding: VisibilityBinding | undefined

function syncOverlayBindingVisibility(): void {
...
const controls = createControlsPane('TSL · Mega Demo', params, (pane, p) => {
  ...
  particlesFolder.addBinding(p, 'particleScale', {
    label: 'scale',
    min: 0.4,
    max: 1.8,
    step: 0.01,
  ...
  morph.addBinding(p, 'holdSeconds', {
    label: 'hold',
    min: 0.5,
    max: 16,
    step: 0.1,
  })
  morph.addBinding(p, 'morphSeconds', {
    label: 'duration',
    min: 0.5,
    max: 12,
    step: 0.1,
...
function syncParams(): void {
  ...
  particles.holdSeconds = p.holdSeconds
  particles.morphSeconds = p.morphSeconds
...
function layoutParticles(): void {
  ...
  particles.mesh.scale.setScalar(baseScale * controls.params.particleScale)
}
...
renderer.setAnimationLoop(() => {
  ...
  if (p.morphEnabled) morphTime += frameDt
    ...
    particles.step(
      {
        ...
        modelMatrix: particles.mesh.matrixWorld,
        cameraRight,
        cameraUp,
        modelRotation,
