...
} from '../../../extras/backgrounds/resolveBackground'
...
// Each overlay style ships its own preset of fluid + visual parameters.
// Selecting a style in the GUI overwrites these fields on `controls.params`
// and refreshes the panel — so the look is genuinely different per style,
// not just a different shader on top of identical fluid behaviour.
interface OverlayPreset {
  ...
  bfecc?: boolean
  /**
   * Per-stroke dye decay. Optional — when absent the runtime mirrors
   * `densityDissipation` (legacy auto-sync). Set explicitly when a style
   * needs dye trails to outlive the density field, like the long
   * watercolour wash on `spread`.
   */
  dyeDissipation?: number
  intensity: number
}

const PRESETS: Record<OverlayStyle, OverlayPreset> = {
...
const DEFAULTS = {
  ...
  overlayStyle: 'default' as OverlayStyle,
...
const OVERLAY_STYLES = Object.keys(PRESETS) as OverlayStyle[]
...
const applyOverlayStyle = (p: typeof DEFAULTS, style: OverlayStyle): void => {
...
const backgroundForStyle = (style: OverlayStyle): BackgroundChoice =>
  style === 'liquidLens' ? 'bright' : 'dark'
...
const controls = createControlsPane('Overlay', initialParams, (pane, p) => {
  ...
  const usesCursorColor = (s: OverlayStyle): boolean =>
    s === 'trail' || s === 'default' || s === 'volumeCursor'
  ...
  const usesVibrance = (s: OverlayStyle): boolean => s !== 'smoke' && s !== 'velocity'
  render
    ...
    .on('change', (ev) => {
      ...
      applyOverlayStyle(p, ev.value as OverlayStyle)
      cursorColorBinding.hidden = !usesCursorColor(ev.value as OverlayStyle)
      vibranceBinding.hidden = !usesVibrance(ev.value as OverlayStyle)
      liquidColorBinding.hidden = ev.value !== 'liquidLens'
      // Auto-switch backdrop to match the selected style.
      switcher.select(backgroundForStyle(ev.value as OverlayStyle))
      pane.refresh()
    })
...
const liquidLensColorize = (dx: number, dy: number): [number, number, number] | undefined => {
...
// Each overlay is a Pass subclass — see `src/lib/effects/overlay/*`. Public
// fields (`intensity`, `vibrance`, `cursorColor`, `time` etc.) get set per
// frame in the loop below; the pass reads its own fluid textures. Effects
// can also be plugged into a three.js `EffectComposer` chain.
type OverlayEffect =
  | DefaultOverlayPass
  | VolumeCursorOverlayPass
  | TrailOverlayPass
...
const overlays: Record<OverlayStyle, OverlayEffect> = {
...
const syncParams = (activeOverlay: OverlayEffect): void => {
