...
function buildOutput(style: OverlayStyle): Node {
  return fluidOverlay(
    style,
    asNode(scenePass),
...
function setOutput(style: OverlayStyle): void {
...
const controls = createControlsPane('TSL · Overlay', initialParams, (pane, p) => {
  ...
  render
    ...
    .on('change', (ev) => {
      const style = ev.value as OverlayStyle
...
function syncParams(): void {
  ...
  asTsl<UniformValue<number>>(opacity).value = p.opacity
