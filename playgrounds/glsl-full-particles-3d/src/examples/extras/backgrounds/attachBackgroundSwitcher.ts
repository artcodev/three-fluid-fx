...
import { persistBackground, type BackgroundChoice } from './resolveBackground'

/** Lazy constructors keyed by choice. The function for a given variant is
 *  only called the first time that variant is selected. */
export type BackgroundFactories = Record<BackgroundChoice, () => Background>

/** Handle returned by {@link attachBackgroundSwitcher}. */
export interface BackgroundSwitcher {
  /** The currently selected background. */
  activeBackground: Background
  /** Advance active/fading backgrounds. Prefer this over `activeBackground.update`. */
  update(dt: number, elapsed: number): void
  /** Programmatically switch to another variant (also updates UI + storage). */
  select(choice: BackgroundChoice): void
  /** Remove the switcher's DOM, dispose all cached backgrounds. */
  dispose(): void
}

interface ExamplePageBackgroundControl {
  setActive: (choice: string) => void
  dispose: () => void
}

const LABELS: Record<BackgroundChoice, string> = {
  dark: '☾ Dark',
  bright: '☀ Bright',
  slideshow: '☷ Slides',
}

const ORDER: readonly BackgroundChoice[] = ['dark', 'bright', 'slideshow']
...
export function attachBackgroundSwitcher(options: {
  scene: Scene
  initial: BackgroundChoice
  factories: BackgroundFactories
  persist?: boolean
  fadeDuration?: number
}): BackgroundSwitcher {
  ...
  const cache = new Map<BackgroundChoice, Background>()
  const visible = new Set<Background>()
  const opacityByBackground = new Map<Background, number>()

  function setOpacity(bg: Background, opacity: number): void {
  ...
  function ensureVisible(bg: Background): void {
    if (visible.has(bg)) return
    visible.add(bg)
    scene.add(bg.mesh)
  }

  function removeVisible(bg: Background): void {
    if (!visible.delete(bg)) return
    scene.remove(bg.mesh)
    setOpacity(bg, 1)
  }

  function get(choice: BackgroundChoice): Background {
  ...
  let activeChoice: BackgroundChoice = options.initial
  let activeBackground: Background = get(activeChoice)
  ...
  function setActive(choice: BackgroundChoice): void {
  ...
  const backgroundControl = pageRuntime.__fluidExamplePage?.registerBackgroundControl?.({
    ...
    active: activeChoice,
    onSelect: (choice) => setActive(choice as BackgroundChoice),
  })
