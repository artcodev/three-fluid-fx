...
export const DEMO_MOTIONS = ['all', ...DEMO_SINGLE_MOTIONS]
const DEMO_MOTION_ALIASES = {
  's-sweep': 'sSweep',
...
export function attachDemoManualTakeover(demo, element, attachManualControl) {
...
function sampleSpiralBloomStroke(t01) {
  const t = smoothstep01(t01)
...
function sampleSoftWhipStroke(t01) {
  const t = smoothstep01(t01)
...
function sampleBreathingOrbitStroke(t01) {
  const t = smoothstep01(t01)
...
function sampleDiagonalRevealStroke(t01) {
  const t = smoothstep01(t01)
...
function sampleSweepSpiralStroke(t01) {
  const t = smoothstep01(t01)
...
function sampleInfinityStroke(t01) {
  const loops = 2.55
...
function sampleSingleDemoStroke(t01, motion) {
  switch (motion) {
    case 'sSweep':
...
function sampleDemoStroke(t01, motion, pauseFraction) {
...
export function createDemoSplatDriver(fluid, options = {}) {
  const params = demoSearchParams()
  ...
  const repeatPauseSeconds = Math.max(
    ...
    readNumberParam(
      params,
      'repeatPause',
      options.repeatPauseSeconds ?? DEMO_REEL_REPEAT_PAUSE_SECONDS,
    ),
  )
